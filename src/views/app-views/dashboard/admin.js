...
export const Admin = () => {
  ...
  const SingleVehicle = async (value) => {
        ...
        const mapData = [
          {
            ...
            live_status: vehicle_live_status(
              processedData?.vehicle_current_status
            ),
            icon_url:
    ...
    if (value && role_id != 6 && currentCustomerUser != "") {
      ...
      const singlevehicles_data = await api
        .post("client_multi_dashboard", customer_data)
        .then((res) => {
          return res;
        })
        .catch((err) => {
      ...
      if (singlevehicles_data?.data?.data) {
        const vehicle_data = singlevehicles_data?.data?.data.filter(
          (item) => item.device_imei === currentVehicle
        );
  ...
  const vehicle_count = async () => {
    if (role_id === 6) {
      ...
      if (selectedCustomer != "") {
        ...
        const count_vehicles = await api
          .post("client_vehicle_count", customer_data)
          .then((res) => {
            return res;
          })
          .catch((err) => {
  ...
  const getAdminList = async () => {
    ...
    const admin_list = await api
      .post("role_based_user_list", admin_data)
      .then((res) => {
        return res;
      })
      .catch((err) => {
  ...
  const getDealerList = async () => {
    ...
    const dealer_list = await api
      .post("role_based_user_list", dealer_data)
      .then((res) => {
        return res;
      })
      .catch((err) => {
  ...
  const getSubDealerList = async () => {
    ...
    const subdealer_list = await api
      .post("role_based_user_list", subdealer_data)
      .then((res) => {
        return res;
      })
      .catch((err) => {
  ...
  const getCustomerList = async () => {
    ...
    const customer_list = await api
      .post("role_based_user_list", customer_data)
      .then((res) => {
        return res;
      })
      .catch((err) => {
    ...
    if (role_id === 6) {
      ...
      if (currentVehicle != "") {
        const vehicle_data = await api
          .get("single_dashboard/" + currentVehicle)
          .then((res) => {
            return res;
          })
          .catch((err) => {
    ...
    try {
      if (role_id === 6) {
          ...
          if (filteredItems) {
            ...
            const processedData = filteredItems?.map((item) => ({
              ...
              icon_url:
                ...
                ".png",
              last_duration: item?.last_duration,
              latitude: item?.lattitute || 0.0,
              longtitude: item?.longitute || 0.0,
              title: item?.vehicle_name || "TEST",
              description: item?.device_updatedtime || "0000-00-00 00:00:00",
              device_time: item?.device_updatedtime || "0000-00-00 00:00:00",
              color: vehicle_color(item?.vehicle_current_status),
        ...
        if (currentCustomerUser) {
          ...
          const customer_vehicles = await api
            .post("client_multi_dashboard", customer_input)
            .then((res) => {
              return res;
            })
            .catch((err) => {
  ...
  const changeCustomer = async (value) => {
    ...
    if (customer_vehicles?.data?.data && status_vehicle != "") {
      ...
      const device_imei =
        currentVehicle != "" ? currentVehicle : filteredItems[0].device_imei;
      const single_vehicle_data = customer_vehicles?.data?.data.filter(
        (item) => item.device_imei === device_imei
      );
                    ...
                    <Col md={12}>
                      <Form.Item
                        ...
                        rules={[
                          {
                            required: true,
                            message: "Dealer Value is Required!",
                          },
                        ]}
                      >
                        {dealerLoading ? (
                    ...
                    <Col md={12}>
                      <Form.Item
                        ...
                        rules={[
                          {
                            required: true,
                            message: "Subdealer Value is Required!",
                          },
                        ]}
                      >
                        <Select
                          showSearch
                          onChange={changeSubDealer}
                    ...
                    <Col md={12}>
                      <Form.Item
                        ...
                        rules={[
                          {
                            required: true,
                            message: "Customer Value is Required!",
                          },
                        ]}
                      >
                        <Select
    ...
    /> */}
                  <List
                    ...
                    renderItem={(item) => (
                      <List.Item
                        className={
                          item?.device_imei == live_vehicle
                            ? "active_vehicle"
                            : "normal_vehicle"
                        }
                        onClick={() => SingleVehicle(item?.device_imei)}
                        value={item?.id}
                        actions={[
            ...
            <Col sm={12} md={18} lg={18}>
              {loading ? <Spin></Spin> : <LiveTracking data={mapvehicleDate} />}
            </Col>
          </Row>
        </Col>
      </Row>
