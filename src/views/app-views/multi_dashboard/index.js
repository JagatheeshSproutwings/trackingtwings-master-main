...
export const MultiDashboard = () => {
  ...
  const vehicle_list = async () => {
    ...
    setChartData([
      vehicle_data?.data?.data?.idle,
      vehicle_data?.data?.data?.running,
      vehicle_data?.data?.data?.stop,
      vehicle_data?.data?.data?.no_data,
      vehicle_data?.data?.data?.inactive,
    ]);
  };
  const multiple_vehicles_count = async () => {
    const multi_vehicle_list = await api
      .get("multi_dashboard")
      .then((res) => {
        return res;
      })
  ...
  const user_list = async (values) => {
    const user_data = await api
      .get("user/show/" + values)
      .then((res) => {
        return res;
      })
      .catch((err) => {
          ...
          >
            ...
            {multiVehicles?.map((vehicle) => (
              <Marker
                ...
                icon={customIcon}
                position={[
                  vehicle?.lattitute || 0.0,
                  vehicle?.longitute || 0.0,
                ]}
              ...
              >
                ...
                >
                  <Row>
                    <Col>
                      <div>
                        Vehicle Name : <b>{vehicle?.vehicle_name || ""}</b>
                      </div>
                      <div>
                        Vehicle Status :{" "}
                        <b>{vehicle?.vehicle_current_status || ""}</b>
                      </div>
                      <div>
                        Main Battery Voltage :{" "}
                        <b>{vehicle?.vehicle_battery_volt || 0} Volts</b>
                      </div>
                      <div>
                        Device Battery Percentage :{" "}
                        <b>{vehicle?.device_battery_percentage || 0} %</b>
                      </div>
                      <div>
                        Speed :<b>{vehicle?.speed} KMPH</b>
                      </div>
                      <div>
                        Last Updated at:<b>{vehicle?.device_updatedtime}</b>{" "}
                      </div>
                      <div>
                        Since : <b>{vehicle?.last_duration} (HH:MM:SS)</b>
                      </div>
                    </Col>
                  </Row>
                </Popup>
